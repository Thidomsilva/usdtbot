...
export default function FanTokensPage() {
  ...
  async function load() {
    try {
      const res = await fetch("/api/fantokens", { cache: "no-store" });
  ...
  return (
    <main style={{ minHeight: "100vh", background: "#f9fafb", fontFamily: "'DM Sans',sans-serif" }}>
      <div style={{ maxWidth: 1160, margin: "0 auto", padding: 24 }}>
        <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <div>
            <div style={{ fontSize: 13, marginBottom: 8 }}>
              <Link href="/" style={{ textDecoration: "none", color: "#6b7280" }}>Voltar para USDT/BRL</Link>
            </div>
        ...
        {data?.summary && (
          ...
          >
            <div>
              <div style={{ fontSize: 12, color: "#6b7280" }}>USD/BRL</div>
              <div style={{ fontSize: 20, fontWeight: 800 }}>{data.summary.usd_brl.toFixed(4)}</div>
            </div>
            <div>
              <div style={{ fontSize: 12, color: "#6b7280" }}>Spread acima de 1%</div>
              <div style={{ fontSize: 20, fontWeight: 800 }}>{data.summary.above_1_pct}</div>
            </div>
            <div>
              <div style={{ fontSize: 12, color: "#6b7280" }}>Spread acima de 3%</div>
              <div style={{ fontSize: 20, fontWeight: 800 }}>{data.summary.above_3_pct}</div>
            </div>
            <div>
              <div style={{ fontSize: 12, color: "#6b7280" }}>Melhor oportunidade</div>
              ...
              >
                ...
                >
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                    <div>
                      ...
                      <div style={{ fontSize: 12, color: "#6b7280", marginTop: 3 }}>{token.team}</div>
                    </div>
                ...
                {isOpen && (
                  <div style={{ borderTop: "1px solid #e5e7eb", padding: 12 }}>
                    {token.best_arb && (
                      <div style={{ fontSize: 12, marginBottom: 10, color: "#6b7280" }}>
                        ...
                        >
                          ...
                          <div style={{ textAlign: "right" }}>
                            {ex.status === "ok" ? (
                              ...
                              <div style={{ fontSize: 12, color: "#6b7280" }}>{ex.error ? ex.error : "Nao listado"}</div>
                            )}
                          </div>
                        </div>
                      ))}
